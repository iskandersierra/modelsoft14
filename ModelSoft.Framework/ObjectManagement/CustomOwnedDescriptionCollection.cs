...
{
  class CustomOwnedDescriptionCollection<TItem, TItemImpl, TOwner> :
    CustomList<TItem>,
    IDescriptionCollection<TItem>
    where TItem : class, IMetaDescription
    where TItemImpl : class, TItem, IDescriptionWithParent
    where TOwner : class
  {
    protected readonly TOwner Owner;

    public CustomOwnedDescriptionCollection(TOwner _Owner, string _CollectionName)
    ...
    protected override bool OnBeforeEnter(TItem item, int pos)
    {
      ...
      if (((TItemImpl)item).Parent != null)
        return false;
      if (((TItemImpl)item).ParentCollection != null)
        return false;
    ...
    protected override void OnAfterEnter(TItem item, int pos)
    {
      ((TItemImpl)item).Parent = Owner;
      ((TItemImpl)item).ParentCollection = this;
      base.OnAfterEnter(item, pos);
    }

    protected override void OnAfterLeave(TItem item, int pos)
    {
      ((TItemImpl)item).Parent = null;
      ((TItemImpl)item).ParentCollection = null;
