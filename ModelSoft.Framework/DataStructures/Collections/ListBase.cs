...
namespace ModelSoft.Framework.DataStructures.Collections
{
  ...
  public abstract class ListBase<T> : CollectionBase<T>, IList<T>, IList
  {
    ...
    {
      return this.First(e => predicate(e));
    }
    ...
    {
      return this.TryGetFirst(predicate, out foundItem);
    }
    ...
    {
      return this.Last(e => predicate(e));
    }
    ...
    {
      return this.TryGetLast(predicate, out foundItem);
    }
    ...
    {
      return this.IndexOf(e => predicate(e));
    }
    ...
    {
      int foundIndex = this.EnumerateRange(index).IndexOf(e => predicate(e));
      if (foundIndex < 0)
        return -1;
      else
    ...
    {
      int foundIndex = this.EnumerateRange(index, index + count).IndexOf(e => predicate(e));
      if (foundIndex < 0)
        return -1;
      else
        return foundIndex + index;
    ...
    {
      return this.LastIndexOf(e => predicate(e));
    }
    ...
    {
      return this.EnumerateRange(0, index).LastIndexOf(e => predicate(e));
    }
    ...
    {
      int foundIndex = this.EnumerateRange(index - count, count).LastIndexOf(e => predicate(e));
    ...
    {
      int foundIndex = this.EnumerateRange(index).IndexOf(item, EqualityComparer<T>.Default);
    ...
    {
      int foundIndex = this.EnumerateRange(index, index + count).IndexOf(item, EqualityComparer<T>.Default);
    ...
    {
      int foundIndex = this.EnumerateRange(0, index).LastIndexOf(item, EqualityComparer<T>.Default);
    ...
    {
      int foundIndex = this.EnumerateRange(index - count, index).LastIndexOf(item, EqualityComparer<T>.Default);
    ...
    {
      return this.AsRange(start, count);
    }
    ...
    private static T ConvertToItemType(string name, object value)
    {
      ...
      {
        throw new ArgumentException(string.Format(@"""{0}"" cannot be converted to type {1}", value, typeof(T)), name);
      }
    }
