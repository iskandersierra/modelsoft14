...
namespace ModelSoft.Framework.DataStructures.Collections
{
  ...
  public class OrderedDictionary<TKey, TValue> : DictionaryBase<TKey, TValue>, ICloneable
  {
    ...
    private static IComparer<KeyValuePair<TKey, TValue>> From(IComparer<TKey> comparer)
    {
      comparer.RequireNotNull("comparer");
      return new ComparisonComparer<KeyValuePair<TKey, TValue>>((x, y) => comparer.Compare(x.Key, y.Key));
    }
    private static IComparer<KeyValuePair<TKey, TValue>> From(Comparison<TKey> comparison)
    {
      comparison.RequireNotNull("comparison");
      return new ComparisonComparer<KeyValuePair<TKey, TValue>>((x, y) => comparison(x.Key, y.Key));
    }
    ...
    public OrderedDictionary(IComparer<TKey> comparer) :
      this(null, comparer, From(comparer))
    {
      if (comparer == null)
    ...
    public OrderedDictionary(Comparison<TKey> comparison) :
      this(null, comparison.AsComparer(), From(comparison))
    {
    }
    ...
    public OrderedDictionary(IEnumerable<KeyValuePair<TKey, TValue>> keysAndValues)
      : this(keysAndValues, Comparer<TKey>.Default)
    {
    }
    ...
    public OrderedDictionary(IEnumerable<KeyValuePair<TKey, TValue>> keysAndValues, IComparer<TKey> comparer)
      : this(keysAndValues, comparer, From(comparer))
    {
      if (comparer == null)
        throw new ArgumentNullException("comparer");
    }
    ...
    public OrderedDictionary(IEnumerable<KeyValuePair<TKey, TValue>> keysAndValues, Comparison<TKey> comparison)
      : this(keysAndValues, comparison.AsComparer(), From(comparison))
    {
    }
    ...
    {
      throw new InvalidOperationException(@"Type ""{0}"" is not cloneable".Fmt(t.FullName));
    }
    ...
    public OrderedDictionary<TKey, TValue> CloneContents()
    {
      ...
      if (!typeof(TKey).IsCloneableType(out keyIsValueType))
        NonCloneableType(typeof(TKey));

      if (!typeof(TValue).IsCloneableType(out valueIsValueType))
    ...
    public sealed override void Add(TKey key, TValue value)
    {
      ...
      if (!added)
        throw new ArgumentException(@"Duplicated key: " + key, "key");
    }
    ...
    public class View : DictionaryBase<TKey, TValue>
    {
      ...
      public sealed override TValue this[TKey key]
      {
        ...
        {
          if (!KeyInView(key))
            throw new ArgumentException(@"Out of range view", "key");
          else
