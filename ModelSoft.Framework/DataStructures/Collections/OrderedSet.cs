...
namespace ModelSoft.Framework.DataStructures.Collections
{
    ...
    ICloneable
  {
    ...
    public OrderedSet(IEnumerable<T> collection, Comparison<T> comparison) :
      this(collection, comparison.AsComparer())
    {
    }
    ...
    public OrderedSet<T> CloneContents()
    {
      ...
      if (!typeof(T).IsCloneableType(out itemIsValueType))
        throw new InvalidOperationException(@"Type ""{0}"" is not cloneable".Fmt(typeof(T).FullName));
    ...
    private void CheckConsistentComparison(OrderedSet<T> otherSet)
    {
      ...
      if (!object.Equals(comparer, otherSet.comparer))
        throw new InvalidOperationException(@"Inconsistent comparison");
    }
    ...
    public void DifferenceWith(OrderedSet<T> otherSet)
    {
      ...
      {
        this.RemoveWhere(item => otherSet.Contains(item));
      }
    }
    ...
    public class View : CollectionBase<T>, ICollection<T>
    {
      ...
      {
        if (!ItemInView(item))
          throw new ArgumentException(@"Out of range view", "item");
        else
